...
exports.main = async (event, context) => {
  ...
  const [letterRes, badgeRes] = await Promise.all([
    db.collection('letters').where({ user_id: openid }).orderBy('updated_at', 'desc').limit(1).get(),
    db.collection('badges').where({ user_id: openid }).get()
  ])

  const albumList = await db.collection('album')
    .where({ user_id: openid })
    .field({ date: true })
    .orderBy('date', 'desc')
    .get()

  const uniqueDays = new Set(albumList.data.map(item => item.date)).size
  const stats = {
    one_day: uniqueDays,
  ...
  await Promise.all(badgeTemplates.map(async template => {
    const current = stats[template.badge_key] || 0
    const level = calcLevel(current, template.target)
    const exist = badgeRes.data.find(item => item.badge_key === template.badge_key)

    if (exist) {
      return db.collection('badges').doc(exist._id).update({
        data: {
          level,
          current,
    ...
    return db.collection('badges').add({
      data: {
        user_id: openid,
  ...
  const latestBadges = await db.collection('badges').where({ user_id: openid }).get()

  return {
    letter: letterRes.data[0] || null,
    badges: latestBadges.data,
    stats,
    checkinDates: Array.from(new Set(albumList.data.map(item => item.date))).sort()
  }
}
